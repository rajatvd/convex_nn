...
class ConvexMLP(Model):
    ...
    def hessian(
        ...
        **kwargs,
        ...
        """Compute the Hessian of the l2 objective with respect to the model weights. If 'block_diagonal' = True,
        then only the blocks along the diagonal of the Hessian are computed.
        :param X: (n,d) array containing the data examples.
        ...
        """
        if block_diagonal:
            H = self._bd_hessian_builder(X, self._signs(X, D))
        else:
            H = self._hessian_builder(X, self._signs(X, D))

            if flatten:
                H = lab.concatenate([entry for entry in H], axis=1)
                H = lab.concatenate([entry for entry in H], axis=1)

        return H / self._scaling(y, scaling)
    ...
    def hessian_operator(
        ...
        **kwargs,
        ...
        """Construct a linear operator to compute Hessian-vector products $H v$ using einsum,
        where the matrix $H$ is the Hessian of the l2 objective with respect to the model parameters.
        All previous comments regarding the efficiency of einsum apply here.
        :param X: (n,d) array containing the data examples.
        :param y: (NOT USED) (n) array containing the data targets.
        :param w: (NOT USED) specific parameter at which to compute the Hessian.
        Defaults to 'None', in which case the current model state is used.
